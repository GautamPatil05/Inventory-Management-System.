...
public class IssueProcess extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String sqlLast = "SELECT * FROM issueprocess ORDER BY srNo DESC LIMIT 1";
			...
			String sql3 = "SELECT quantity FROM processmaterial WHERE name=?";
			...
			String sql2 = "UPDATE processmaterial SET quantity=? where name=?";
			...
			// Updating in Log
			String sql = "INSERT INTO issueprocess(userID, srNo, name, quantity, issuer, date, time)  value(?, ?, ?, ?, ?, ?, ?)";
			...
			if(i > 0) {
				 ...
				 String message = "Material Issued and Log Updated";
			        ...
			        out.println("window.location.href='" + request.getContextPath() + "/issueProcess.jsp';");
			        out.println("</script>");
			}
			else {
