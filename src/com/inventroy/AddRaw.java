...
public class AddRaw extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String sqlLast = "SELECT * FROM rawmaterial ORDER BY srNo DESC LIMIT 1";
		...
		String sql = "INSERT INTO rawmaterial (srNo, name, quantity, unit, costPerUnit, userID) SELECT ?, ?, ?, ?, ?, ? WHERE EXISTS (SELECT id FROM admin WHERE id = ?)";
			...
			if (j > 0) {
		        ...
		        out.println("window.location.href='" + request.getContextPath() + "/addRaw.jsp';");
		        out.println("</script>");
			}
